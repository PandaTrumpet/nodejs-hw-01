...
export const generateContacts = async (number) => {
  try {
    ...
    const oldData = await fs.readFile(PATH_DB, 'utf-8');
    const oldParseData = JSON.parse(oldData);
    // console.log(oldParseData);

    const newData = [...oldParseData, ...data];
    // console.log(newData);
    const endData = JSON.stringify(newData, null, 2);
    await fs.writeFile(PATH_DB, endData, 'utf-8');
    console.log(endData);
  } catch {
    console.log('Error');
  }
